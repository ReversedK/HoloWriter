...
define_zome! {
	...
	functions: [
        ...
        get_item: {
            inputs: |item_addr : Address|,
            outputs: |result: ZomeApiResult<GetHoloJsEntry>|,
    ...
    traits: {
        hc_public [get_linked_items,create_collection, get_item, add_item, get_list,link_bidir,delete_item,update_item,unlink_items,link_items]
    }
}
...
fn handle_get_item(item_addr: Address)->ZomeApiResult<GetHoloJsEntry>{
    let the_entry = hdk::utils::get_as_type::<HoloJsEntry>(item_addr.to_owned())?;
          
    Ok(GetHoloJsEntry { addr : item_addr.to_owned(), item: the_entry.item,entity_type:the_entry.entity_type})
}
